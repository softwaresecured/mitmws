...
import com.mitmws.pki.PKIProvider;
...
public class HttpClient {
        ...
        if ( host != null && port >= 0 ) {
            ...
            if ( msg.isSslEnabled() ) {
                try {
                    ...
                    socket = (Socket) MitmWsSocketFactory.upgradeSocketConnection(socket,msg,applicationConfig);
    ...
    public Socket connectDirect(String host, int port, boolean tls) throws HttpClientException {
        try {
            if ( tls ) {
                socket = MitmWsSocketFactory.getEncryptedSocket(host,port,applicationConfig);
            }
            else {
                socket = MitmWsSocketFactory.getPlaintextSocket(host,port,applicationConfig);
            }
