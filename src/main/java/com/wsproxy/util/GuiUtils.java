...
public final class GuiUtils {
    ...
    /*
    public static void updateFrameViewerByMsgId(PnlFrameViewer pnlFrameViewer, String messageId, Project project ) {
        try {
            WsProxyTrafficDataReader wsProxyTrafficDataReader = new WsProxyTrafficDataReader(project);
            pnlFrameViewer.jtxtFramePayload.setText("");
            WebsocketTrafficRecord rec = null;
            rec = wsProxyTrafficDataReader.getWebsocketTrafficRecord(messageId,null);
            if ( rec != null ) {
                String maskStr = "--";
                ...
                String frameStr = String.format("F:%d/R1:%d/R2:%d/R3:%d/OP:%s/M:%s",
                        ...
                        maskStr);
                pnlFrameViewer.pnlWebsocketFrameViewerToolbar.jlblFrameProperties.setText(frameStr);
                pnlFrameViewer.setFrameViewerPayload(payloadViewText);
                //pnlTraffic.pnlWebsocketTraffic.pnlFrameViewer.jtxtFramePayload.setText(new String(rec.getFrame().getPayloadUnmasked()));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    */
    /*
    public static void updateFrameViewer(PnlFrameViewer pnlFrameViewer, JTable tblWebsocketTraffic, Project project ) {

        int rowId = tblWebsocketTraffic.getSelectedRow();
        if ( rowId >= 0 ) {
            String messageId = tblWebsocketTraffic.getValueAt(rowId, 0).toString();
            if ( messageId != null ) {
                updateFrameViewerByMsgId(pnlFrameViewer,messageId,project);
            }
        }
    }
    */
